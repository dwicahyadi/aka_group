        ...
        <div class="row align-items-center  mb-30">
            <div class="col-md-6">
                <div class="title d-flex align-items-center flex-wrap mb-30">
                    <h2 class="mr-40">Pajak Kendaraan</h2>
                </div>
            </div>
        </div>
        <!-- end row -->
    ...
    @else
        ...
        <div class="card-style mb-30">
            ...
            <form action="{{ route('tax.store') }}" method="post" >
                ...
                <div class="input-style-1">
                    <label for="license_number">Jadwal Pajak Berikutnya</label>
                    <div class="row">
                        <div class="col-3">
                            <input type="date" @error('next_tax_date') class="form-control is-invalid" @enderror name="next_tax_date"
                                   id="date" value="{{ old('next_tax_date') ?? \Carbon\Carbon::now()->add('year', 1)->format('Y-m-d') }}" min="date('Y-m-d')" required>
                            @error('next_tax_date')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                        </div>
