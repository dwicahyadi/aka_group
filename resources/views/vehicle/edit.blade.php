        ...
        <div class="row align-items-center  mb-30">
            <div class="col-md-6">
                <div class="title d-flex align-items-center flex-wrap mb-30">
                    <h2 class="mr-40">Edit Kendaraan</h2>
                </div>
            </div>
        </div>
        <!-- end row -->
            ...
            <div class="row">
                    ...
                    <div class="row">
                        <div class="col-3">
                            <input type="text" @error('code') class="form-control is-invalid" @enderror name="code"
                                   id="code" value="{{ old('code') ?? $vehicle->code }}" placeholder="Kode unik kendaraan" required>
                            @error('code')
                    ...
                    <div class="row">
                        <div class="col-3">
                            <input type="text" @error('brand') class="form-control is-invalid" @enderror name="brand"
                                   id="brand" value="{{ old('brand') ?? $vehicle->brand }}" placeholder="Merk" required>
                            @error('brand')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                        </div>
                        <div class="col-4">
                            <input type="text" @error('model') class="form-control is-invalid" @enderror name="model"
                                   id="model" value="{{ old('model') ?? $vehicle->model }}" placeholder="Model/Type" required>
                            @error('model')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                        </div>
                        <div class="col-3">
                            <input type="number" @error('year') class="form-control is-invalid" @enderror name="year"
                                   id="year" value="{{ old('year') ?? $vehicle->year }}" placeholder="Tahun Kendaraan" required>
                            @error('year')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        <div class="col-3">
                            <input type="text" @error('license_number') class="form-control is-invalid" @enderror name="license_number"
                                   id="license_number" value="{{ old('license_number') ?? $vehicle->license_number }}" required>
                    ...
                    <div class="row">
                        <div class="col-6">
                            <input type="text" @error('vin') class="form-control is-invalid" @enderror name="vin"
                                   id="vin" value="{{ old('vin') ?? $vehicle->vin }}" required>
                            @error('license_number')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                        </div>
                ...
                <div class="mb-30">
                    ...
                    <input type="file" @error('image') class="form-control is-invalid" @enderror name="image"
                           id="image" value="{{ old('image') ?? $vehicle->image }}" accept="image/*" onchange="loadFile(event)">
                    @error('image')
                    <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                    @enderror
                    <br>
                    <img id="output" src="{{ \Illuminate\Support\Facades\Storage::url($vehicle->image) }}">
                </div>
                            ...
                            <div class="select-position">
                                <select @error('driver_id') class="form-control is-invalid" @enderror name="driver_id"
                                    ...
                                    @foreach($drivers as $id => $name)
                                        <option @if($vehicle->driver_id == $id) selected @endif value="{{ $id }}">{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('driver_id')
                        ...
                        <div class="select-style-1">
                            ...
                            <div class="select-position">
                                <select @error('tax_officer_id') class="form-control is-invalid" @enderror name="tax_officer_id"
                                        id="tax_officer_id">
                                    <option value="">Pilih Petugas Pajak</option>
                                    @foreach($tax_officers as $id => $name)
                                        <option @if($vehicle->tax_officer_id == $id) selected @endif  value="{{ $id }}">{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
